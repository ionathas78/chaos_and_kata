# Earth Reference Material

A practical source library for the martial maneuvers integrated into the **Earth** form.

This is not a claim that Earth reproduces any source form exactly. The project borrows real movement vocabulary, then links and inflects it according to Earth's training and elemental grammar.

> **Borrow the technique. Inflect the phrase.**

These references answer: **“What does that maneuver look like again?”** They prioritize clear demonstrations and useful mechanical context. Yoga and ordinary strength positions such as Tree, Warrior III, lunges, and planks are not included unless Earth uses them in a specifically martial adaptation.

---

## Wing Chun

### Pak Sau / advancing entry

**Used in:** Four Corners

Earth uses Pak Sau as a compact clearing/control action while advancing into the punch rather than as a separate pose.

- Howcast — *How to Do a Pak Sau | Wing Chun*  
  https://www.youtube.com/watch?v=rozXKxxWhAs

The demonstration is particularly useful because it shows Pak Sau applied around the opponent's elbow and then combined with forward pressure and punching.

---

## Kung Fu / Wushu

### Hou Sao Tui / Back Sweep

**Used in:** Four Corners, B → C

Earth uses a deliberately controlled approximately 180° rear sweep to reverse facing. Standard modern Wushu demonstrations may continue through a larger arc, so study the sweep mechanics rather than treating the competition form's exact rotation as mandatory for Earth.

- UK Shaolin — *Kung Fu back sweep | Hou Sao Tui | Shaolin Kung Fu*  
  https://www.youtube.com/watch?v=yN9UUZAt8xE

---

## Panther / Leopard Vocabulary

### Panther FLOW / Leopard-inspired low structure

**Used in:** Four Corners exit, both long journeys, ground transitions

Earth's Panther is intentionally **Panther/Leopard-inflected**, not presented as one canonical stance from a single lineage. The useful vocabulary is low loaded geometry, angular entry, hip/waist rotation, elbows and knees, low-line attack, compression and extension, and springing out of the legs.

Because Panther/Leopard practice varies considerably by lineage, no single video should be treated as defining Earth's Panther. References should be labeled by lineage and used for specific mechanical ideas rather than as proof of one universal Panther form.

### Panther Rising Elbow

**Used in:** Panther → Muay Thai Guard → Teep transitions

Earth uses Panther's low compressed geometry as the launch structure for a rising elbow. The legs and hips drive the body upward while the elbow travels on a compact ascending line. The movement begins with Panther/Leopard-inflected compression and springing mechanics, then resolves into an upright Muay Thai guard.

There is no claim that **Panther Rising Elbow** is a canonical named Panther technique. It is project terminology for a hybrid transition whose launch is Panther-inflected and whose elbow mechanics are informed by Muay Thai **Sok Ngat / upward elbow**.

- YOKKAO — *How to Throw a Right Uppercut Elbow | Muay Thai Fundamentals*  
  https://www.youtube.com/watch?v=phOXoJeOf7Q

- Fight Encyclopedia — *Standard Sok Ngat / Uppercut Elbow*  
  https://fightencyclopedia.com/techniques/strike/elbow-strike/uppercut-elbow/sok-ngat/standard-sok-ngat

**Earth phrase:**

**Panther FLOW → Panther Rising Elbow SNAP → Muay Thai Guard FLOW → Teep SNAP → recoil to Crane / Thai Check**

The important mechanical idea is that Panther does not rise and then throw the elbow. **The elbow is how Panther rises.** The recovery creates the guard, the guard provides the platform for the Teep, and the Teep recoil creates Crane.

---

## Muay Thai

### Sok Ngat / Uppercut or Rising Elbow

**Used in:** Panther Rising Elbow

Sok Ngat supplies the clearest real-world reference for the compact upward elbow action used when Earth rises out of Panther. Study the coordinated leg and hip drive and the upward elbow path rather than copying a complete Muay Thai combination.

- YOKKAO — *How to Throw a Right Uppercut Elbow | Muay Thai Fundamentals*  
  https://www.youtube.com/watch?v=phOXoJeOf7Q

- Fight Encyclopedia — *Standard Sok Ngat / Uppercut Elbow*  
  https://fightencyclopedia.com/techniques/strike/elbow-strike/uppercut-elbow/sok-ngat/standard-sok-ngat

### Muay Thai Guard

**Used in:** Rising Elbow → Teep transition

The rising elbow resolves into an upright guarded structure rather than a static pose. The guard is a FLOW landmark that organizes the body for the Teep.

### Teep / Front Push Kick

**Used in:** Panther-to-Crane transitions

From the upright Muay Thai guard, the lead knee gathers and the foot drives forward to manage distance. The kick retracts completely and its recoil is captured as Crane / Thai Check rather than immediately returning the foot to the floor.

- Muay Thai Basics — *The Teep or Front Push Kick*  
  https://www.youtube.com/watch?v=epJCYeLRVmg

- Muay Thai — *Teep / Front Push Kick* alternate demonstration  
  https://www.youtube.com/watch?v=45SMzAtCPGI

- Evolve MMA — *3 Muay Thai Counters You Need To Know*  
  https://evolve-mma.com/blog/3-muay-thai-counters-you-need-to-know/

The Evolve reference is useful for the authentic **check → same-leg teep without first putting the foot down** relationship. Earth reverses the emphasis by letting the completed Teep recoil become the raised-knee Crane structure.

### Thai Check / raised-knee guard

**Used in:** Crane / Thai Check

Earth borrows the raised-knee defensive structure as the martial foundation of Crane. The support leg stays alive rather than locked, the knee gathers high, and the hands remain in a functional guard.

- Muay Thai Bible — *Shin Block / Check*  
  https://muaythaibible.com/th/techniques/shin-block-check

---

## Shotokan Karate

### Mae Geri → Oi Zuki → Gyaku Zuki

**Used in:** Third Journey

This is the core striking sentence of the Third Journey: front kick, land into the same-side stepping/lunge punch, then rotate through the rooted stance into the opposite-hand reverse punch.

- Shotokan Karate Online — *2nd Kyu Syllabus Mae Geri Oi Zuki Gyaku Zuki Brown Belt Basics*  
  https://www.youtube.com/watch?v=EgMY9Fo2OIQ

This is the cleanest single reference for the exact three-technique relationship we use.

- Karate of Stanford — basic Shotokan kick reference, including **Mae Geri** and the importance of kick retraction  
  https://karate.groups.stanford.edu/resources/kihon/kicks

### Kiba-dachi / Horse

**Used in:** Root, Four Corners, Third Journey

The Third Journey borrows Tekki's low lateral horse-stance geometry and hip-driven close-range character.

- Shotokan Karate Online — *Tekki Shodan Step-by-Step Tutorial*  
  https://www.shotokankarateonline.com/blog/tekki-shodan-step-by-step-tutorial/

- KarateDecoded — *Tekki Shodan — Full Kata Guide*  
  https://karatedecoded.com/kata/tekki-shodan/

### Nami-gaeshi / Returning Wave

**Used in:** Third Journey

From the low lateral root, the weight shifts onto one leg while the other foot folds/lifts inward and then returns to rebuild the stance. Earth reads it primarily as removal/checking of the leg against a low attack, while recognizing that karate bunkai gives the motion several possible applications.

- Farmhouse Shotokan Karate Club — *Nami-Gaeshi (returning wave)*  
  https://www.youtube.com/watch?v=o6_cHYPERUY

- Karate of Stanford — lists **nami-gaeshi / nami-ashi-geri** as Returning Wave  
  https://karate.groups.stanford.edu/resources/kihon/kicks

- KarateForums discussion — *My Top 10 Applications for Nami-Gaeshi*, including kick/sweep avoidance and shin-block/check interpretations  
  https://karateforums.com/topic/44639/my-top-10-applications-for-nami-gaeshi/

### Hooking Elbow → Ground Punch

**Used in:** Third Journey

Earth links the returning Nami-gaeshi foot into a close hooking elbow and then carries the attack downward into the Ground Punch before rebuilding Horse.

This linkage is derived from our study of Tekki/Naihanchi close-range geometry rather than copied as a fixed named combination from a single source.

- International Neoclassical Karate & Kobudo Society — *Naihanchi Kata Comparison*  
  https://www.inkks.org/articles/naihanchi-kata-comparison

The comparison is useful for seeing returning-wave movements embedded among close-range forearm, elbow, and short turning-punch actions.

---

## Tai Chi

### Cloud Hands

**Used in:** Return to Root, immediately after Third Journey

Earth uses Cloud Hands as a continuous lateral weight-transfer cooldown after the final percussive Shotokan phrase. It softens the concluding Horse without abandoning martial movement, restores circular motion and smooth breathing, and gradually narrows the structure toward Natural before the closing fold and Commencement.

**Reference still to be selected.** Add a demonstration after we have watched it and confirmed that its weight transfer is useful for our version.

### Tai Chi Commencement

**Used in:** Return to Root

Earth borrows the familiar rising-and-sinking arm action as the final controlled release back into Natural stance.

**Reference still to be selected and vetted.**

---

## Reference Policy

A reference belongs here when it does at least one of these jobs:

1. Clearly demonstrates a named maneuver used in Earth.
2. Shows the mechanical relationship between maneuvers integrated into Earth.
3. Documents the martial vocabulary or lineage from which an Earth adaptation was drawn.

Prefer references we have actually watched. If a useful source disappears, replace it rather than accumulating dead links.

When our choreography deliberately changes a source maneuver, say so explicitly. The purpose of this file is to preserve Earth's **movement ancestry**, not to imply that Strength Kata is a traditional form from any one martial art.

Each future elemental form should maintain its own reference-material file so the source libraries remain compact and easy to work with.
